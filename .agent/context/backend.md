...
Required fields: `documents[]` with `source` and `chunks`, plus
`total_documents` and `total_chunks`.
...
- `actions[]` with `tool`, `status`, and `result`
- `requires_human`
- `sources[]`
- `trace_id`
...
- Read safe error fields from the root or object-valued `detail` envelope.
